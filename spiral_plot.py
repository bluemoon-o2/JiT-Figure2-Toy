...
# -------------------
def train_one(D, data2d, config, param="x", num_steps=4000, batch_size=512, lr=1e-3):
    """
    Trains a denoising MLP on a random projection of the 2D spiral.

    The 2D dataset is projected into `R^D` using a random column-orthogonal
    matrix `P` obtained via QR decomposition. The model is trained to predict
    one of three parameterizations:
        - `"x"`: predict the clean sample `x0`
        - `"eps"`: predict the diffusion noise `ε`
        - `"v"`: predict the velocity `v = √ᾱ ε − √(1−ᾱ) x0`

    Parameters:
        D: Target feature dimension for the random projection
        data: Base 2D dataset as `np.ndarray` of shape `(N, 2)`
        config: Dictionary with diffusion schedule (`alphas_bar`) and device
        param: One of `{"x", "eps", "v"}` selecting the training target
    ...
    Returns:
        `(model, P)` where:
            - `model` is the trained `DenoiseMLP`
            - `P` is the `D×2` projection matrix used to map 2D → D
    """
    device = config['device']
    # projection: R^2 -> R^D with column-orthogonal matrix P (D x 2)
    rand = torch.randn(D, 2)
    P, _ = torch.linalg.qr(rand)      # Orthonormal columns (D, 2)
    P = P.to(device)

    x0_D = torch.from_numpy(data2d).to(device) @ P.T                           # Project to (N, D)
...
def main():
    ...
    for D in Ds:
        for param in params:
            model, P = train_one(D, base2d, config, param=param, num_steps=4000)
